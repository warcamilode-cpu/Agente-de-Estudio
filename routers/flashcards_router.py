...
def flashcards_pendientes(materia_id: int | None = Query(None)):
    params: list = [str(date.today())]
    where = "proximo_repaso <= ?"
    if materia_id is not None:
        where += " AND materia_id = ?"
        params.append(materia_id)
    with db() as conn:
        rows = conn.execute(
            f"SELECT * FROM flashcards WHERE {where} ORDER BY proximo_repaso", params
        ).fetchall()
    return [dict(r) for r in rows]


@router.get("")
def listar_flashcards(materia_id: int | None = Query(None)):
    with db() as conn:
        if materia_id is not None:
            rows = conn.execute(
                "SELECT * FROM flashcards WHERE materia_id = ? ORDER BY creada_at DESC", (materia_id,)
            ).fetchall()
        else:
...
def crear_flashcard(body: FlashcardIn):
    with db() as conn:
        cur = conn.execute(
            "INSERT INTO flashcards (pregunta, respuesta, materia_id) VALUES (?, ?, ?)",
            (body.pregunta, body.respuesta, body.materia_id),
        )
...
def registrar_respuesta(card_id: int, body: RespuestaIn):
    ...
    with db() as conn:
        ...
        nuevo_intervalo, nuevas_reps, nuevo_ef = calcular_siguiente_repaso(
            body.calificacion, card["intervalo"], card["repeticiones"], card["factor_facilidad"]
        )
        proximo = (date.today() + timedelta(days=nuevo_intervalo)).isoformat()
        conn.execute(
            "UPDATE flashcards SET intervalo=?, repeticiones=?, factor_facilidad=?, proximo_repaso=? WHERE id=?",
...
def editar_flashcard(card_id: int, body: FlashcardIn):
    with db() as conn:
        conn.execute(
            "UPDATE flashcards SET pregunta=?, respuesta=?, materia_id=? WHERE id=?",
            (body.pregunta, body.respuesta, body.materia_id, card_id),
        )
        row = conn.execute("SELECT * FROM flashcards WHERE id = ?", (card_id,)).fetchone()
    if row is None:
        raise HTTPException(status_code=404, detail="Flashcard no encontrada")
